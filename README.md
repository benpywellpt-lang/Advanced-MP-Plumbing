...
- `script.js` - scroll engine, chapter rail, nav, call bar, hero tape swap, guided
  "Learn more" tour, mobile sheet, form validation.
...
and headless Edge's `--screenshot`; use `?p=` and `?focus=` instead, which keep scroll at 0.
For real scrolled captures and anything that relies on IntersectionObserver (the call bar),
drive headless Edge over the DevTools protocol: `node ../.claude/tools/cdp-scroll-check.mjs <outDir>`.
...
3. The Gas Safe badge (`assets/img/gas-safe.jpg`) is the 213x236 file from the client's
   old site. Ask them for the current artwork from their Gas Safe account if they have it.
